...
def greedy_nn(g: nx.Graph, distance_matrix:np.matrix):
    ...
    while graph.number_of_edges() != int(g.number_of_nodes()*0.5):
...
def find_nn(node:int, nodes_list:list, distance_matrix:np.matrix):
    nn= nodes_list[0]
    min_distance = np.min(distance_matrix[node-1][np.nonzero(distance_matrix[node-1])])
    for n in nodes_list[1:]:
        distance = np.min(distance_matrix[node-1][np.nonzero(distance_matrix[node-1])])
...
def greedy_cycle(g: nx.Graph, distance_matrix:np.matrix):
    ...
    while graph.number_of_edges() != int(g.number_of_nodes()*0.5):
        ...
        _,_,shortest_loop = find_shortest_loop(unused_nodes[0],g.edges(),distance_matrix)
